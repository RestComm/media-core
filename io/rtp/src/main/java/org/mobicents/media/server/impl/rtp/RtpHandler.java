	...
	public boolean canHandle(byte[] packet, int dataLength, int offset) {
        ...
        if (dataLength >= RtpPacket.FIXED_HEADER_SIZE) {
            ...
            if (RtpPacket.VERSION == version) {
                /*
                 * When RTP and RTCP packets are multiplexed onto a single port, the RTCP packet type field occupies the same
                 * position in the packet as the combination of the RTP marker (M) bit and the RTP payload type (PT). This field
                 * can be used to distinguish RTP and RTCP packets when two restrictions are observed:
                 * 
                 * 1) the RTP payload type values used are distinct from the RTCP packet types used.
                 * 
                 * 2) for each RTP payload type (PT), PT+128 is distinct from the RTCP packet types used. The first constraint
                 * precludes a direct conflict between RTP payload type and RTCP packet type; the second constraint precludes a
                 * conflict between an RTP data packet with the marker bit set and an RTCP packet.
                 */
