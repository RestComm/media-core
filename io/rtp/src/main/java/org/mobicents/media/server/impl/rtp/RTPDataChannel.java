...
public class RTPDataChannel {
    ...
    public void bind(boolean isLocal) throws IOException, SocketException {
        ...
        if (channelsManager.getIsControlEnabled()) {
            if (!isLocal)
                ...
                udpManager.bindLocal(rtcpChannel, rtpChannel.socket().getLocalPort() + 1);
        }
    }
    ...
    private class TxTask {
        ...
        public void performDtmf(Frame frame) {
            ...
            oobPacket.wrap(false, AVProfile.telephoneEventsID, sn++, dtmfTimestamp, ssrc, frame.getData(), frame.getOffset(),
                    frame.getLength());
        ...
        public void perform(Frame frame) {
            ...
            rtpPacket.wrap(false, fmt.getID(), sn++, timestamp, ssrc, frame.getData(), frame.getOffset(), frame.getLength());
