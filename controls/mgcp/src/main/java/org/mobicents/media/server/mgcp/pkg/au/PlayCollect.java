 ...
 * 
 * Plays a prompt and collects DTMF digits entered by a user. If no digits are
 * entered or an invalid digit pattern is entered, the user may be reprompted
 * and given another chance to enter a correct pattern of digits. The following
 * digits are supported: 0-9, *, #, A, B, C, D. By default PlayCollect does not
 * play an initial prompt, makes only one attempt to collect digits, and
 * therefore functions as a simple Collect operation. Various special purpose
 * keys, key sequences, and key sets can be defined for use during the
 * PlayCollect operation.
 * 
 * 
 * @author oifa yulian
 */
public class PlayCollect extends Signal {
	...
	private class PromptHandler implements PlayerListener {
		 ...
		 * 
		 * @param signal
		 *            the play record signal instance
		 */
