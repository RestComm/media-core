...
public class AuditConnectionCmd extends Action {
    ...
    /**
     * The <b>ConnectionId</b> parameter is the identifier of the audited connection, within the context of the specified
     * endpoint.
     */
    private Parameter connectionId;
    /**
     * The <b>EndpointId</b> parameter specifies the endpoint that handles the connection. The wildcard conventions SHALL NOT be
     * used.
     */
    ...
    /**
     * The (possibly empty) <b>RequestedInfo</b> describes the information that is requested for the ConnectionId within the
     * EndpointId specified. The following connection info can be audited with this command:<br>
     * 
     * CallId, NotifiedEntity, LocalConnectionOptions, Mode, RemoteConnectionDescriptor, LocalConnectionDescriptor,
    ...
    protected void reset() {
        ...
        this.queryCallId = false;
        this.queryNotifiedEntity = false;
        this.queryLocalConnectionOpts = false;
        this.queryMode = false;
        this.queryRemoteConnectionDes = false;
        this.queryLocalConnectionDes = false;
        this.queryConnectionParams = false;
    }
    ...
    private class Audit extends Task {
        ...
        public long perform() {
            ...
            connection = endpoints[0].getConnection(connectionId.getValue().hexToInteger());
            if (connection == null) {
        ...
        private void auditRequestedInfo(Collection<Text> requestedParams, MgcpConnection connection) {
            for (Text param : requestedParams) {
                if (param.equals(Parameter.CALL_ID)) {
                    ...
                    notifiedEntity = endpoints[0].getRequest().getCallAgent();
    ...
    private class Respond extends Task {
        ...
        public long perform() {
            ...
            /*
             * The AuditConnection response will in turn include information about each of the items auditing info was requested
             * for.
             * 
             * If no info was requested and the EndpointId is valid, the gateway simply checks that the connection exists, and
             * if so returns a positive acknowledgement.
             */
            if (requestedInfo != null) {
                if (queryCallId) {
