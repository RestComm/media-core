 ...
 *
 */
public abstract class DeprecratedAbstractMgcpConnection implements MgcpConnection {
    ...
    public DeprecratedAbstractMgcpConnection(int identifier, int callId,  int halfOpenTimeout, int openTimeout, MgcpEventProvider eventProvider, ListeningScheduledExecutorService executor) {
        // Connection State
    ...
    public void updateMode(ConnectionMode mode, FutureCallback<Void> callback) {
        synchronized (this.stateLock) {
            if (MgcpConnectionState.CLOSED.equals(this.state)) {
                Throwable t = new IllegalStateException("Cannot update mode because connection is closed.");
    ...
    final class MgcpConnectionTimer implements Callable<Integer> {
        ...
        @Override
        public Integer call() {
            DeprecratedAbstractMgcpConnection.this.notify(DeprecratedAbstractMgcpConnection.this, new RtpTimeoutEvent(getIdentifier(), this.timeout));
            return timeout;
        }
    ...
    final class MgcpConnectionTimerCallback implements FutureCallback<Integer> {
        ...
        public void onSuccess(Integer result) {
            ...
            if (!MgcpConnectionState.CLOSED.equals(state)) {
                close(new FutureCallback<Void>() {

                    @Override
                    public void onSuccess(Void result) {
                        log().warn("Connection " + getHexIdentifier() + " was closed in elegant manner after timeout.");
                        
                    }

                    @Override
                    public void onFailure(Throwable t) {
                        log().warn("Could not close connection " + getHexIdentifier() + " in elegant manner after timeout.", t);
                    }
                });
