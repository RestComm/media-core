 ...
 *
 */
public class ActivateMixerAction
    ...
    private static final Logger log = Logger.getLogger(ActivateMixerAction.class);
    
    static final MgcpEndpointAction INSTANCE = new ActivateMixerAction();
    ...
    public void execute(MgcpEndpointState from, MgcpEndpointState to, MgcpEndpointEvent event, MgcpEndpointTransitionContext context, MgcpEndpointFsm stateMachine) {
        ...
        globalContext.getMixer().start();

        if (log.isTraceEnabled()) {
            EndpointIdentifier endpointId = globalContext.getEndpointId();
            log.trace("Endpoint " + endpointId + " activated its in-band mixer.");
        }

        // Register connection to out-of-band mixer
        globalContext.getOobMixer().start();

        if (log.isTraceEnabled()) {
            EndpointIdentifier endpointId = globalContext.getEndpointId();
            log.trace("Endpoint " + endpointId + " activated its out-of-band mixer.");
        }
    }
