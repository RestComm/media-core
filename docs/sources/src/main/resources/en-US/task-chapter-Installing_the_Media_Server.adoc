...
The &MMS_LONG_NAME;distribution is available in two forms: Standalone and Embedded in JBoss Application Server 5.
The standalone version is preferred for carrier production deployment while JBoss Application Server 5 version is convenient for development 
...
Alternatively, the source code for the Media Server can be obtained by checking it out from its repository using the Subversion version control system ( 		). You can later run a build using Maven.
...
:leveloffset: 1
include::mob-task-section-Java_Development_Kit-Installing_Configuring_and_Running.adoc[]
:leveloffset: 0

[[_itms_binary_jboss_media_server_installing_configuring_and_running]]
== JBoss Application Server 5.x.y embedded Media Server BinaryDistribution: Installing, Configuring and Running

&MMS_LONG_NAME;either comes bundled with the JBoss Application Server or Standalone.
This section details how to install the &MMS_LONG_NAME;that comes bundled with JBoss Application Server 5.
For installation of Standalone &MMS_LONG_NAME;, refer to <<_itms_binary_standalone_media_server_installing_configuring_and_running>>		
...
+
----
C:Users<user>My Downloadsms-<version>jar -xvf ""
----


. It is recommended that the folder holding the JBoss embedded Media Server files (in this example, the folder named [path]_
  ms-
  <version>
  _						) is moved to a user-defined location for storing executable programs.
  For example, the [path]_Program Files_						folder. 
. Consider deleting the archive, if free disk space is an issue. 
+
----
C:Users<user>My Downloadsms-<version>delete ""
----
...
.Procedure: Running the JBoss embedded Media Server on
...
in the /conf/bootstrap/ directory. For
