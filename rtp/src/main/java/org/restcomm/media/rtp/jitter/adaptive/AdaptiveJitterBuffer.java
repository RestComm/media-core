 ...
 */
public class AdaptiveJitterBuffer implements JitterBuffer, Serializable {
    ...
    private TreeSet<Frame> queue = new TreeSet<Frame>(new FrameComparator());
    ...
    private final static Logger logger = Logger.getLogger(AdaptiveJitterBuffer.class);
     ...
     */
    public AdaptiveJitterBuffer(RtpClock clock) {
        this.rtpClock = clock;
    }
     ...
     * @param clock the rtp clock.
     */
    public AdaptiveJitterBuffer(RtpClock clock, PlayoutStrategy adaptionStrategy) {
        this.rtpClock = clock;
        this.playoutStrategy = adaptionStrategy;
    }
        ...
        if (bufferListener != null) {
            if (!useBuffer) {
                if (logger.isDebugEnabled()) {
                    logger.debug("Jitter Buffer is ready! [duration=" + bufferDuration + "ms, frames=" + queue.size() + "]");
                }
        ...
        if (playoutOffset >= 0) {
            ...
            try {
                playoutTimer.schedule(new TimerTask() {
                    ...
                    public void run() {
                        if (logger.isDebugEnabled()) {
                            logger.debug("Jitter Buffer is ready to be played! [duration=" + bufferDuration + "ms, frames="
                                    + queue.size() + "]");
                        }
                        bufferListener.onFill();
                    }
                ...
                if (logger.isDebugEnabled()) {
                    logger.debug("Jitter Buffer is scheuled! [playoutOfgset = " + playoutOffset + " duration=" + bufferDuration
                            + "ms, frames=" + queue.size() + "]");
                }
            } catch (Exception e) {
    ...
    public String toString() {
        return "AdaptiveJitterBuffer [isn=" + isn + ", arrivalDeadLine=" + arrivalDeadLine + ", dropCount=" + dropCount
                + ", bufferDuration=" + bufferDuration + ", currentTransit=" + currentTransit + ", currentJitter="
                + currentJitter + ", useBuffer=" + useBuffer + "]";
    }
