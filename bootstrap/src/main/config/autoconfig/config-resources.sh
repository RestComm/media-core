...
configResourcePooling() {
    ...
    xmlstarlet ed -u "/mediaserver/resources/localConnection/@poolSize" -v "$local_connection_count" \
        ...
        -u "/mediaserver/resources/dtmfGenerator/@poolSize" -v "$dtmf_generator_count" $file
}
...
configAudioCache() {
    ...
    xmlstarlet ed -u "/mediaserver/resources/player/cache/cacheSize" -v "$cache_size" \
        -u "/mediaserver/resources/player/cache/cacheEnabled" -v "$cache_enabled" $file
}
...
configDtmfDetector() {
    ...
    xmlstarlet ed -u "/mediaserver/resources/dtmfDetector/@dbi" -v "$dbi" $file
}
