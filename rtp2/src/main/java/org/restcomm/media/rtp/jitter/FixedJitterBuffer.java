...
public class FixedJitterBuffer implements JitterBuffer {
    ...
    private final static Logger log = Logger.getLogger(FixedJitterBuffer.class);
    ...
    // buffer's monitor
    private final Set<JitterBufferObserver> observers;
    ...
    private void safeWrite(RtpPacket packet, RTPFormat format) {
        if (this.format == null || this.format.getID() != format.getID()) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Format changed! [" + this.format.toString() + "]");
            }
        }
        ...
        if (f != null) {
            ...
            if (duration < 0 && queue.size() > 1) {
                log.warn("Something messy happened. Reseting jitter buffer!");
                reset();
                return;
            }
            ...
            if (queue.size() > QUEUE_SIZE) {
                if (log.isTraceEnabled()) {
                    log.trace("Jitter Buffer overflow! [duration=" + duration + "ms, frames=" + queue.size() + "]");
                }
                dropCount++;
    ...
    public void write(RtpPacket packet, RTPFormat format) {
        ...
        if (format == null) {
            if (log.isTraceEnabled()) {
                log.trace("No format specified. Packet dropped!");
            }
            return;
        }
        ...
        try {
            ...
            if (log.isTraceEnabled()) {
                log.trace("Could not aquire write lock for jitter buffer. Dropped packet.");
            }
        } finally {
            if (locked) {
                this.lock.unlock();
    ...
    public Frame read(long timestamp) {
        ...
        try {
            ...
            if (log.isTraceEnabled()) {
                log.trace("Could not acquire reading lock for jitter buffer.");
            }
            this.ready.set(false);
        } finally {
            if (locked) {
    ...
    private Frame safeRead() {
        if (queue.size() == 0) {
            this.ready.set(false);
            if (log.isTraceEnabled()) {
                log.trace("Jitter Buffer is empty. Consumer will wait until buffer is filled.");
            }
            return null;
        }
        ...
        if (queue.size() == 0) {
            this.ready.set(false);
            if (log.isTraceEnabled()) {
                log.trace("Read last packet from Jitter Buffer.");
            }
            // arrivalDeadLine = 0;
    ...
    public void reset() {
        ...
        try {
            ...
            if (log.isTraceEnabled()) {
                log.trace("Could not acquire lock to reset jitter buffer.");
            }
        } finally {
            if (locked) {
                lock.unlock();
    ...
    public void restart() {
        ...
        if (log.isDebugEnabled()) {
            log.debug("Restarted jitter buffer.");
        }
    }

    @Override
    public void observe(JitterBufferObserver observer) {
        boolean added = this.observers.add(observer);
        if(added && log.isTraceEnabled()) {
            log.trace("Registered new observer " + observer.toString() + ". Count: " + this.observers.size());
        }
    }

    @Override
    public void forget(JitterBufferObserver observer) {
        boolean removed = this.observers.remove(observer);
        if(removed && log.isTraceEnabled()) {
            log.trace("Unregistered observer " + observer.toString() + ". Count: " + this.observers.size());
        }
    }

    @Override
    public void notify(JitterBufferEvent event) {
        Iterator<JitterBufferObserver> iterator = this.observers.iterator();
        while (iterator.hasNext()) {
            JitterBufferObserver observer = (JitterBufferObserver) iterator.next();
            observer.onJitterBufferEvent(this, event);
        }
    }
