...
public class MessageIntegrityAttribute extends StunAttribute implements
	...
	protected MessageIntegrityAttribute(char attributeType) {
	...
	public void setRemoteKey(byte[] remoteKey) {
		if (remoteKey == null) {
			...
			this.remoteKey = remoteKey.clone();
		}
	}
	...
	public byte[] encode(byte[] data, int offset, int length) {
		...
		if (Message.isRequestType(msgType)) {
			key = getRemoteKey();
			// key =
			// stunStack.getCredentialsManager().getRemoteKey(username,media);
		...
		this.hmacSha1Content = calculateHmacSha1(data, offset, length, key);
